...
local garages = { 
	[1] = {camera = {x = -330.945, y = -135.471, z = 39.01, heading = 102.213}, inside = {x = -337.3863,y = -136.9247,z = 38.5737, heading = 269.455}},
	[2] = {camera = {x = 737.09, y = -1085.721, z = 22.169, heading = 114.86},  inside = {x = 733.69,y = -1088.74, z = 21.733, heading = 270.528}},
	[3] = {camera = {x = -1154.902, y = -2011.438, z = 13.18, heading = 95.49}, inside = {x = -1155.077,y = -2006.61, z = 12.465, heading = 162.58}},
	[4] = {camera = {x = 1177.98, y = 2636.059, z = 37.754, heading = 37.082},  inside = {x = 1174.823,y = 2637.807, z = 37.045, heading = 181.19}},
	[5] = {camera = {x = 105.825, y = 6627.562, z = 31.787, heading = 266.692}, inside = {x = 108.842,y = 6628.447, z = 31.072, heading = 45.504}},
	[6]=  {camera = {x = -215.518, y = -1329.135, z = 30.89, heading = 329.092},inside = {x = -212.368,y = -1325.486, z = 30.176, heading = 141.107}}
}
	...
	if DoesEntityExist(veh) then
		...
		respray = LSCMenu.categories:addSubMenu("RESPRAY", "Respray", "Transforms vehicle appearance.",true)
			pcol = respray:addSubMenu("PRIMARY COLORS", "Primary color",  nil,true)
				...
				ccol = respray:addSubMenu("PEARLECSENT", "Pearlecent", nil, true)
				ccol:addSubMenu("PEARLECSENT", "Pearlecent", nil, true)
	
				for n, c in pairs(LSC_Config.prices.pearl.colors) do
					local btn = ccol.Pearlecent:addPurchase(c.name, LSC_Config.prices.pearl.price)
    ...
    while true do
        ...
        if inside == false then
            ...
            if IsPedSittingInAnyVehicle(ped) then
                ...
                if DoesEntityExist(veh) and GetPedInVehicleSeat(veh, -1) == ped and (IsThisModelACar(GetEntityModel(veh) or IsThisModelAQuadbike(GetEntityModel(veh) or IsThisModelABike(veh)))) then
                    for i, pos in ipairs(garages) do
                        if GetDistanceBetweenCoords(pos.inside.x, pos.inside.y, pos.inside.z, GetEntityCoords(ped)) <= f(5) then
...
function CheckPurchases(m)
	...
	elseif name == "pearlecent" then
        for i, b in pairs(m.buttons) do
            if b.colorindex == myveh.extracolor[1] then
                b.sprite = "garage"
            else
                b.sprite = nil
            end
